...
export default function Passos(props: PassosProps) {
  ...
  const childrenArray = Children.toArray(props.children).filter(isValidElement);
  const passoAtualElement = childrenArray[passoAtual] as ReactElement | undefined;

  return (
    <div className="flex-1 flex flex-col gap-10 w-full">
      <div className="self-center">{renderizarLabels()}</div>
      <div>{passoAtualElement}</div>
